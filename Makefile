...
clean-build: ## remove build artifacts
	...
	rm -fr dist/
	rm -fr .eggs/
	find . -name '*.egg-info' -not -path './venv_make/*' -not -path './venv_windows/*' -not -path './venv/*' -exec rm -fr {} +
	find . -name '*.egg' -not -path './venv_make/*' -not -path './venv_windows/*' -not -path './venv/*' -exec rm -f {} +
